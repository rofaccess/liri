...
task :update_agent_installer do
  ...
  liriagent_lib_path = File.expand_path("../liriagent/lib")
  ...
  system("echo '#{Liri::VERSION}' > ../liriagent/bin/.liri-version")
  # Obtiene la dirección de la gema compilada
  compiled_gem_file_path = File.join(Dir.pwd,"/pkg/#{Liri::NAME}-#{Liri::VERSION}.gem")
...
task :compress_agent_installer do
  ...
  agent_installer_compressed_path = File.expand_path("../")
...
# Publica la gema después de compilar. Esto todavía no se probó
# > rake publish
